...
for context in [{State:"unavailable",Detail:"context lookup failed"},
    ...
    Assert(!ScheduleReaction("reaction_send",3) && !ActiveReactionJob && StartRequests=1,
        "failed context stops before preparing or sending a reaction")
    Assert(LastReactionResult.Reason==context.State
...
for mode in ["reaction_capture","reaction_check","reaction_send"] {
    for replaced in [false,true] {
        ...
        Assert(!escaped && !scheduled && StartRequests=1,"context exception is handled without scheduling or retry: " label)
        if replaced {
            Assert(ActiveReactionJob=StartReplacement && StartReplacement.Phase="queued"
            ...
            Assert(!ActiveReactionJob && LastReactionResult.Reason="unavailable" && LastReactionResult.Detail="context request exception",
                "context exception is preserved as a failure before starting: " label)
            Assert(PaletteStart.Enabled && ManagementItemButtons[1].Enabled,"context failure leaves controls usable: " label)
        }
    }
    StartContextReply := 0, StartContextReplace := false, StartRequests := 0
    Critical("On")
    try {
        Assert(ScheduleReaction(mode,30) && ActiveReactionJob.Mode=mode && StartRequests=1,"new scheduled request can start after context failure: " mode)
        CancelReaction()
        Assert(!ActiveReactionJob,"successful retry can be cancelled before any browser action: " mode)
    ...
    for mode in ["queued","reaction_capture","reaction_send"] {
        for point in ["status","overlay"] {
            for scenario in ["failure","replacement_failure","replacement","cancelled"] {
                ...
                try {
                    ...
                    if scenario="failure" {
                        ...
                        if scenario="cancelled" {
                            Assert(!ActiveReactionJob && LastReactionResult.Reason="cancelled","cancelled start keeps its cancellation result: " label)
                            StartPreservedResult := LastReactionResult
                            ReplaceStartingJob()
                        }
                        Assert(ActiveReactionJob=StartReplacement && StartReplacement.Phase="queued" && StartReplacement.Remaining=7,"successor keeps ownership and state: " label)
                        Assert(LastReactionResult=StartPreservedResult && ReactionExecutionStatus.Phase!="finished" && ReactionExecutionStatus.Message=="successor pending","old start cannot replace the successor's result: " label)
                ...
                if StartReplacement
                    Assert(ActiveReactionJob=StartReplacement && StartReplacement.Remaining=7,"successor is unchanged after the timer deadline: " label)
                if ActiveReactionJob
                    FinishReactionJob(ActiveReactionJob)
                Assert(StartOriginal.Phase="finished","original start is terminal: " label)
            }
        }
    }
} finally {
