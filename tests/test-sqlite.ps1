...
try {
    ...
    SqlCheck(reuse.Scalar("SELECT value FROM sample WHERE id=4")=="日本語👏"
    ...
    SqlCheck(reuse.Scalar('SELECT value FROM "日本語🎉"')=="本文","direct execution and prepared queries share Unicode identifiers")
...
try {
    ...
    SqlCheck(failed && snapshotStore.Saved=initialBaseline && !snapshotStore.Saved.Scopes.Count && !snapshotStore.Saved.Preferences,"first save failure retains the empty schema baseline")
    SqlCheck(snapshotStore.Db.Scalar("SELECT COUNT(*) FROM scopes")="0" && snapshotStore.Db.Scalar("SELECT COUNT(*) FROM items")="0" && snapshotStore.Db.Scalar("SELECT COUNT(*) FROM preferences")="0","first save rolls back every data table")
    snapshotStore.SaveAll(snapshotState)
    SqlCheck(snapshotStore.Saved!=initialBaseline && snapshotStore.Saved.Preferences && snapshotStore.Saved.Scopes.Has("@shared"),"retry publishes one complete baseline")
    for scenario in [{Loaded:true,Point:"version"},{Loaded:true,Point:"commit"},{Loaded:false,Point:"version"},{Loaded:false,Point:"commit"}] {
        ...
        SqlCheck(failed,"read failure reaches " point)
        SqlCheck(snapshotStore.Saved=baselineBefore,"failed read retains the entire prior comparison baseline: " point)
        SqlCheck(snapshotStore.Db.Scalar("SELECT body FROM items WHERE id='snapshot-item'")==snapshotState.SharedDanmakuItems[1].Text,"failed read never modifies stored data: " point)
        ...
        SqlCheck(true,"save and reload succeed after a failed read: " point)
    }
...
try {
    ...
    for change in [{Property:"InputProfileId",Value:"preference-profile"},{Property:"AutoMode",Value:0},
        ...
        SqlCheck(Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))-before=1,"one preference changes one stored row: " change.Property)
        preferenceStore.SavePreferences(preferenceState)
        SqlCheck(Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))-before=1,"same preference value does not write again: " change.Property)
        VerifySettingsRoundTrip(preferenceState,preferenceStore.Load())
        SqlCheck(true,"all values survive preference update and reload: " change.Property)
    }
    for index, definition in ShortcutDefinitions() {
        ...
        before := Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))
        preferenceStore.SavePreferences(preferenceState)
        SqlCheck(Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))-before=1,"one key changes one stored row: " definition.Id)
        preferenceStore.SavePreferences(preferenceState)
        SqlCheck(Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))-before=1,"same key value does not write again: " definition.Id)
        VerifySettingsRoundTrip(preferenceState,preferenceStore.Load())
        SqlCheck(true,"all values survive key update and reload: " definition.Id)
    }
    ...
    VerifySettingsRoundTrip(preferenceState,preferenceStore.Load())
    SqlCheck(true,"library save changes selection and items while ignoring unsaved caller preference edits")
    callerState.InputProfileId := "preference-profile"
    before := Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))
    preferenceStore.SavePreferences(callerState)
    SqlCheck(Integer(preferenceStore.Db.Scalar("SELECT total_changes()"))-before=1,"saved baseline stays independent of the loaded preference object and key map")
    VerifySettingsRoundTrip(callerState,preferenceStore.Load())
    SqlCheck(true,"explicit preference save persists the caller edits after library save")
...
CloseSettingsStore()
FileAppend("PASS: " SqliteChecks " SQLite storage, delta, failure and recovery checks; no application startup`n","*")
ExitApp()
