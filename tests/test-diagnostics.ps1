...
for point in ["construction","snapshot","viewport"] {
    ...
    Assert(failed,"diagnostic creation preserves the failure: " point)
    Assert(ProbeDiagnosticHwnd && !DllCall("IsWindow","Ptr",ProbeDiagnosticHwnd),"failed diagnostic creation destroys its unpublished window: " point)
    if point = "viewport"
        Assert(ProbeDiagnosticViewport.Disposed,"failed diagnostic creation releases viewport callbacks")
    ProbeDiagnosticFailure := "", ProbeDiagnosticViewport := 0
    diagnostic := CreateDiagnosticPanel()
    try {
        diagnostic.Refresh.Call()
        Assert(DllCall("IsWindow","Ptr",diagnostic.Window.Hwnd) && !diagnostic.Viewport.Disposed,"diagnostic creation and refresh retry after failure: " point)
    } finally {
...
try {
    ...
    Assert(ProbeDiagnosticReports.Length=1 && InStr(ProbeDiagnosticReports[1],"(focused)"),"diagnostic copy starts with the completed display")
    ...
    Assert(failed && A_IsCritical=beforeCritical,"failed diagnostic paint preserves the cause and restores interrupt state")
    Assert(!ProbeDiagnosticPaintEnabled && !ProbeDiagnosticPaintCopied,"incomplete diagnostic paint cannot be copied")
    ProbeDiagnosticCopy.Call()
    Assert(!ProbeDiagnosticCopyButton.Enabled && ProbeDiagnosticReports.Length=1,"failed diagnostic paint keeps copying unavailable")
    ProbeDiagnosticPaintFailure := false
    diagnostic.Refresh.Call()
    ProbeDiagnosticCopy.Call()
    Assert(ProbeDiagnosticCopyButton.Enabled && ProbeDiagnosticReports.Length=2 && InStr(ProbeDiagnosticReports[2],"(cleared)"),"successful retry publishes and enables the new diagnostic report")
    ProbeDiagnosticFailure := "snapshot", failed := false
    try diagnostic.Refresh.Call()
    catch as failure
        failed := failure.Message == "fixture diagnostic snapshot failure"
    ProbeDiagnosticCopy.Call()
    Assert(failed && ProbeDiagnosticCopyButton.Enabled && ProbeDiagnosticReports.Length=3
...
FileAppend("PASS: " Checks " diagnostic construction, publication and recovery checks; no browser or clipboard operations`n","*")
ExitApp()
