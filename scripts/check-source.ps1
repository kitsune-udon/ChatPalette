...
if ($version -notmatch '^\d+\.\d+\.\d+(?:-[A-Za-z0-9.-]+)?$' -or
    [IO.File]::ReadAllText((Join-Path $ProjectRoot 'README.md')) -notmatch ('現在のバージョン：\*\*'+[regex]::Escape($version)+'\*\*')) { throw 'README/VERSION mismatch' }
