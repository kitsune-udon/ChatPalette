...
Help(*) {
    ...
    topics.UseTab(1)
    view.AddText("x28 y52 w456 h210","1. YouTubeのチャット欄・コメント欄をクリック。`n`n2. " ShortcutKeyLabel(GetShortcutKey("palette")) "でパレットを開く。`n`n3. 弾幕を選び「選んだ弾幕を入力」。`n`n4. 内容を確認して、YouTube側で送信。`n`nキーの一覧と変更は「管理・ヘルプ」→「ショートカットを管理」から行います。")
    topics.UseTab(2)
    ...
    topics.UseTab(3)
    view.AddText("x28 y52 w456 h230","初回の準備`n操作・サポート → ① ボタンを設定… → YouTubeの♡にマウスを重ねる。②で送信せずに確認できます。`n`n実行`nパレットで種類・回数・間隔を選び、開始。3秒以内に♡へマウスを重ねます。" ShortcutKeyLabel(GetShortcutKey("stop")) "で停止できます。`n`n標準設定はキー実行に適用。今回の設定を保存するには「標準設定に保存」を使います。")
    topics.UseTab(4)
    view.AddText("x28 y52 w456 h230",EffectiveShortcutSummary() "`n`nキーの変更は「ショートカットを管理」から行います。")
    topics.UseTab()
