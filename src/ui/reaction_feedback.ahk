...
RenderLatestReactionStatus() {
    ...
    phase := snapshot.Phase, final := phase = "finished"
    remaining := 100-(AppClockMs()-lastRenderedAt)
    if !final && phase = lastPhase && remaining > 0 {
...
RenderReactionOverlay(snapshot) {
    final := snapshot.Phase = "finished"
    ReactionOverlay.Text.Text := snapshot.Message
    ReactionOverlay.Hint.Text := ReactionProgressHint(snapshot)
    ReactionOverlay.Stop.Visible := !final
    ReactionOverlay.Stop.Enabled := !final
}

ReactionProgressHint(snapshot) {
    return snapshot.Phase = "finished" ? "4秒後に表示を消します。結果は" ShortcutKeyLabel(GetShortcutKey("palette")) " → 管理・ヘルプ → リアクションの実行結果。"
...
ShowReactionProgress(*) {
    ...
    if !ReactionOverlay {
        ...
        try {
            ...
            ReactionOverlay := {Window:view,Text:text,Hint:hint,Stop:stop}
        } catch as failure {
            if IsSet(view)
                view.Destroy()
            throw failure
    ...
    snapshot := ReactionExecutionStatus
    RenderReactionOverlay(snapshot)
    PresentWindow(ReactionOverlay.Window,"x20 y20",0,false)
...
ShowReactionDetails(*) {
    ShowInfoDialog("リアクションの実行結果",BuildReactionDetails.Bind(LastReactionResult))
}
BuildReactionDetails(result, details) {
