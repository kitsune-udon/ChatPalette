...
自動選択では、oEmbedのチャンネルキーを配信者の`Channel`と`FindProfileByChannel`で厳密に照合します。`Channel`が連携の正本で、別の索引や配信者名の類似検索は使いません。 動画確認の例外は未取得の応答へ揃え、古いチャンネル・投稿者・動画を残しません。表示・復帰時は失敗してもパレットを開き、共通弾幕と失敗理由を表示します。保存済みの手動選択は変更せず、次回の判別で復旧します。空のチャンネルは一致とせず、重複した連携は保存・読込時に拒否します。`GetPaletteInputProfile`は選択中の配信者の連携先を確認し、有効な配信者を返します。表示・編集対象の解決はこの結果を使い、IDと有効フラグを別々に渡して再検索しません。
