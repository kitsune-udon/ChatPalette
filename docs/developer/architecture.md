...
アプリ名: ChatPalette。リポジトリ名: `ChatPalette`。起動ファイル: `main.ahk`。パネル呼び出し: Ctrl＋Alt＋Q。
...
INI全体をメモリで組み立て、一時ファイルへUTF-16 BOM付きで一括書き込みしてから置換する。選択中と同じ投稿者を選び直した場合は保存しない。書式と保存成功後にメモリへ反映する契約は変えない。
...
登録オブジェクトごとに検証済みの識別情報のコピーと検索条件を準備し、最大8件まで再利用する。登録オブジェクトは直接変更せず、読み込み・再登録時に新しいオブジェクトへ置き換える。画面要素は毎回表示・有効状態・識別情報を確認し、1秒ごとの再検索も維持する。
