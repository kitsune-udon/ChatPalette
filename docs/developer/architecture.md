...
`ShortcutDefinitions`が全10操作の初期キー・名称・適用範囲を定義します。 実行時の正本は全10操作を持つ`ShortcutKeys`です。リアクション専用のキー変数・登録APIはありません。`ApplyPreferences`が登録の変更と既定値の反映を扱い、設定変更時は保存も行います。起動・再読込は保存せず同じ適用処理を通り、初回のキー登録は`InstallApplicationShortcuts`が担います。DB形式3の`preferences.reaction_key`と9件の`shortcut_bindings`は、リポジトリの読み書きでのみ分離・結合します。`main.ahk`は固定HotIf定義を持たず、`InstallApplicationShortcuts`が保存済みの割当を登録します。パレットはグローバル、停止はリアクションジョブ存在時、他の操作はブラウザー最前面時だけ有効です。キー変更時は正規化した全割当の重複を検証し、変更された旧キーを解除してから新キーを登録するため、キー同士の交換も可能です。保存失敗・登録失敗時は新キーを解除して旧キーを復元します。GUIとヘルプは現在の割当から表示します。

`page_actions.ps1`の`chat_focus`は、表示中・編集可能なチャット欄を一意に探索します。既存の入力分類と祖先情報を共有し、動画と最前面を再確認してUIAの`SetFocus`を呼び、同じ要素にフォーカスできたことを確認します。フォーカス操作は再送せず、反映の観測だけを最大350ms待ちます。クリアはその動画を固定して`verify_chat`で再検証した後、AHK側で全選択・Backspaceを送ります。コメント欄は受け付けず、クリップボードやEnterは使いません。検証とキー送信の間は不可分ではなく、実際の入力と同じ制約があります。

フォーカス操作だけは`ActiveChatFocus`が1件の保留弾幕を所有します。キー解放待ちの前から受付を開始し、5秒の期限とキー押下時のライブラリを保持します。別操作は通常の操作ポリシーで拒否します。フォーカス成功時にワーカーが返す一度限りの識別情報を`verify_chat`へ渡し、元のUIA要素との一致を確認します。確認後に本文・前面・期限を再検証して入力し、終了時は成功・失敗を問わず保留状態を解放します。入力直前の検証と文字入力の間は、通常の入力と同様に不可分ではありません。
...
ジョブは`CreateReactionJob`で同じ項目を持つ形に生成します。状態は`queued`（キー解放待ち）、`waiting`（カウントダウン・登録待ち）、`running`、`stopping`、`finished`です。進行状態の変更は`SetReactionJobPhase`、中止要求は`CancelReaction`、タイマー停止と現在ジョブの解除は`FinishReactionJob`に集約しています。ショートカット受付・予約開始時の前面化・進捗表示・タイマー設定に失敗した場合も`FinishReactionJob`で解放してから失敗を通知します。登録待ちは、次回のタイマー設定まで成功した場合だけジョブを維持します。応答が再待機対象でも、その後の表示やタイマー設定が失敗した場合は終了します。古いジョブを終了しても、新しいジョブのタイマーや参照は解除しません。表示用状態はジョブから作り、表示文言・表示用Phaseで実行の終了を判断しません。
...
`LoadSettings`は読込に失敗した接続を閉じてからエラーを返します。修復後の再試行では新しい接続で読み込みます。
...
`CreateLibrarySnapshot`と`CreateSettingsSnapshot`は編集用の独立した複製を返します。内部コマンドの`CreateLibraryDraft`は未変更の項目を共有するため、外部向けスナップショットと区別します。
