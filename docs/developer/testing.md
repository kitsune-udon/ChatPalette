...
このエラーハンドラーはテスト入口だけに追加します。画面テストと計測は`InitializeApplication(false)`で起動失敗時の復旧ダイアログ待ちを避けます。テスト入口へ製品用のコマンド引数は渡しません。製品の`main.ahk`と`--check`・`--smoke`・不正引数の拒否は起動テストで別途確認します。通知表示や処理中のボタン状態は、初期化不要の試験に持ち込まず、対応する画面テストで確認します。
