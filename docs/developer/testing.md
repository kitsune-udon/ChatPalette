...
現行の一括実行は32テスト群です。アプリ・SQLite・GUIの一部は実際に動かし、ブラウザー入力・リアクション送信・ネットワークは代替処理に置き換えます。成功は、ブラウザー全種類との互換性や実際のYouTube受理数の証明にはなりません。
...
| 編集ダイアログ | [test-app-editors.ps1](../../tests/test-app-editors.ps1) | 保存・キャンセル・再表示・フォーカス復元 |
...
| 実キーの経路 | [test-page-hotkeys.ps1](../../tests/test-page-hotkeys.ps1) | main.ahkの割当からF／C／Eを発火し、隔離GUIでフォーカス・実際のクリア・表示要求を確認 |
...
従来の大きな`test-app.ps1`は7つの独立したシナリオへ分割しました。共通の`app-fixture.ps1`は各呼び出しで新しい隔離フォルダー、旧INIの合成データ、偽ワーカー、明示的なテスト状態を作り、新しいAHKプロセスで初期化します。前のテストが追加・変更したデータには依存しません。配置などの補助関数は必要なシナリオだけが定義します。
...
UI構築中の割り込み、再描画回数、移行途中のプロセス停止、別プロセスのIPC応答には、限定したソース注入が残っています。これらは通常の機能差し替えとは別の障害注入です。新しい汎用フックを本体の各行へ増やさず、注入対象が変わったらその検査の目的を確認してください。
...
追加したページ操作は、隔離テストでチャット限定の検証、動画・前面・対象の変更、失敗時のクリア抑止、クリックや送信を行わないこと、キー競合を確認しています。実キー経路の7検査も成功し、クリアは隔離GUIだけで実行しました。BraveのYouTubeページでは、ネイティブ処理によるフォーカス成功と表示用ボタンへのマウス移動成功を確認しました。リアクションUIの見た目は自動検証していません。旧版の起動プロセスを再起動した後、利用者から3機能が想定どおり動作しているとの確認を得ました。
